...
- 영문 브랜드 톤이 깔끔하고 모던. 한국 테크 제품의 일반적 결(예: Toss, Linear)
...
- 기존 운세 앱의 "올해 운세" 카드는 봤는데, 막상 *"오늘 팀장이랑 또 부딪혔어"* 같은 일상 상황은 어디서 물어볼지 모름
- 최근 이직 고민 + 3개월 전 이별
...
| 기존 운세 앱 | ✅ | △ (사전 설계 콘텐츠 안에서만) | ❌ (단발 리포트) | 콘텐츠 카드 |
| ChatGPT 프롬프팅 | ❌ | ✅ | ❌ | 자유 대화 (그러나 명식 hallucination) |
| 점집 (오프라인) | ✅ | ✅ | ❌ | 1회 대면 |
| **폴라리스** | ✅ | ✅ | ✅ | 누적 자유 대화 |

기존 사주 서비스는 사전 설계된 콘텐츠 범위 안에서만 작동하며, 의도적으로 비워둔 두 칸 — **임의 상황 대응 + 누적 관계** — 이 존재한다. 폴라리스는 이 두 칸을 채우는 포지션이다.
...
> 운세는 절대적인 정답을 맞히는 개념이 아니라, 사용자 상황과 맥락을 이해하고 방향을 제시하는 도구.
> 폴라리스는 그 도구가 콘텐츠 카드가 아니라, 일상의 고민을 받아주는 코치의 형태일 때 더 잘 작동한다고 봅니다.

## 왜 사주 도메인인가
- 한국 사주 시장 규모 + 20-30대 성장세
- LLM 등장 후에도 UX는 10년 전과 동일 → 재정의 기회
- AI를 통한 동양 철학의 개인화된 코칭 전환 가능성
...
### 왜 기존 운세 서비스와 다른 길을 갔는가
기존 운세 서비스는 구조화된 콘텐츠(올해 운세, 연애운, 성향 분석)로 **사전 설계된 자기 이해 리포트**를 제공합니다.
폴라리스는 *"임의의 일상 상황 + 누적 메모리"*로 **일상 상담 코치**의 자리를 채웁니다.

기존 서비스가 의도적으로 비워둔 두 영역 — **임의 상황 대응**(콘텐츠 카테고리 밖의 일상 고민)과
**누적 관계**(단발 리포트가 아닌 follow-up 대화) — 을 폴라리스가 채웁니다.
